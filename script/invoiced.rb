...
Entry.find(:all).group_by {|a| a.spent_on.beginning_of_week.strftime('%Y-%m-%d')}.inject({}) {|m, (month, coll)| m[month] = coll.sum(&:hours_spent); m}.sort{|a,b| a[0] <=> b[0]}.each do |time, hours|
  puts "\t#{time}:\t#{hours}"
end